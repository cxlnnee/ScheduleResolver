...
class AiScheduleService extends ChangeNotifier {
  ...
  final String _apiKey = 'AIzaSyCZRspXZKdMqU6sgYNKosR0xpsHZJFICns';
  ...
  Future<void> analyzeSchedule(List<TaskModel> tasks) async {
    ...
    try {
          ...
          Your job is to analyze these tasks, identify any overlaps or conflicts in their start and end times, and suggest a better banced schedule, 
    ...
    ScheduleAnalysis _parseResponse (String fullText) {
      ...
      for (var section in sections) {
        ...
        else if (section.startsWith('Ranked Task')) conflicts = section.replaceFirst('Ranked Task', '').trim();
        else if (section.startsWith('Recommended Schedule')) conflicts = section.replaceFirst('Recommended Schedule', '').trim();
        else if (section.startsWith('Explanation')) conflicts = section.replaceFirst('Explanation', '').trim();
      }
